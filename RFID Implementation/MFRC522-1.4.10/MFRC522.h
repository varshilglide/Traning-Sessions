...
public:
	...
	MFRC522(uint8_t chipSelectPin, byte resetPowerDownPin);
	...
	StatusCode MIFARE_Ultralight_Write(uint8_t page, byte *buffer, uint8_t bufferSize);
	...
	StatusCode PCD_NTAG216_AUTH(uint8_t *passWord, byte pACK[]);
...
protected:
	...
	StatusCode MIFARE_TwoStepHelper(uint8_t command, byte blockAddr, int32_t data);
};
